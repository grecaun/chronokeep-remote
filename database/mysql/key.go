...
func (m *MySQL) GetAccountKeys(key string) ([]types.Key, error) {
	db, err := m.GetDB()
	if err != nil {
		return nil, err
	}
	...
	res, err := db.QueryContext(
		ctx,
		"SELECT account_id, key_name, key_value, key_type, reader_name, valid_until FROM api_key a WHERE key_deleted=FALSE AND EXISTS (SELECT * FROM api_key b WHERE a.account_id=b.account_id AND b.key_value=?);",
		key,
	)
	if err != nil {
